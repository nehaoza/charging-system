...
public class BusDepotService {
  ...
  public int getChargeDeferential(final ElectronicVehicle electronicVehicle, final Supplier<LocalTime> time) {
  ...
  private int chargeSchoolBus(final int currentCharge, final LocalTime localTime) {
    ...
    if (localTime.isBefore(LocalTime.parse("08:00:00"))) {
      ...
      charge = 50;
        ...
        timeIsBetween(localTime, "18:00:00", "23:59:59")) {
      charge = 30;
    }
    return charge;
  }

  private boolean timeIsBetween(final LocalTime localTime, final String afterHour, final String beforeHour) {
    return localTime.isAfter(LocalTime.parse(afterHour)) && localTime.isBefore(LocalTime.parse(beforeHour));
  }

  private void validateInput(final ElectronicVehicle electronicVehicle, final Supplier<LocalTime> time) {
